...
const myLongId = decodeToken(token).longId
const chats = ref([])

// in props
const props = defineProps({ mySessionId: Number })
/**
 * METHODS
 */
...
function sendMessage() {
  ...
  client.value.publish({
    destination: `/app/live/sendDm/${props.mySessionId}`,
    headers: {
      Authorization: `Bearer ${token}`
    },
    body: JSON.stringify({
      content: newMessage.value,
      sender: myLongId // 발신자명을 설정할 수 있습니다.
    })
  })
  newMessage.value = ''
  const end = document.getElementById('end')
